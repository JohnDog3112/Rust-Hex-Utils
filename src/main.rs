...
mod hex_coord;
use draw_options::{Lines, Intersections, Point};
...
fn main() {
    ...
    let beginning_point = Point::SinglePoint(Color::WHITE, 7.0);
    let ending_point = Point::SinglePoint(Color::BLACK, 7.0);
    let middle_point = Point::SinglePoint(Color::from_rgba8(255, 100, 0, 255), 4.0);

    let intersection = Intersections::EndsAndMiddle(beginning_point, ending_point, middle_point);
    ...
    let line_options = Lines::Gradient(Color::WHITE, Color::BLACK);
