use tiny_skia::{Pixmap, Stroke, LineCap, Color, LineJoin};

use crate::{draw_options::{Intersections, Lines}, pattern_utils::{Coord, Angle, Direction, DynamicList, HexCoord}};

use super::{point::draw_points, draw_monocolor::draw_monocolor_lines, draw_gradient::draw_gradient_lines, draw_segments::draw_segment_lines};
...
impl Pattern {
    ...
    pub fn draw_pattern(&self, pixmap: &mut Pixmap, origin: HexCoord, scale: f32, line_thickness: f32, line_options: &Lines, point_options: &Intersections) {
        ...
        match line_options {
            ...
            Lines::Gradient(colors, segs_per_color, bent) => {
                if colors.len() < 2 {
                    ...
                    end_colors = (
                        colors[0],
                        draw_gradient_lines(&self, pixmap, &stroke, origin, scale, colors, *segs_per_color, *bent)
                    );
                }
            },
        ...
        match point_options {
            ...
            Intersections::EndsAndMiddle(beginning, end, middle) => {
                ...
                draw_points(&vec![beginning_point], pixmap, origin, scale, &beginning);
                if beginning_point != ending_point {
                    draw_points(&vec![ending_point], pixmap, origin, scale, &end);
                }
